...
import '../../../../../../data/data_sources/v3/input_result_design_source.dart'
    show InputResultDataSourceLocal;
...
class ObjectDataCubit extends Cubit<ObjectInputDataState> {
  ...
  final InputResultDataSourceLocal _dataSourceLocal;
  ...
  ObjectDataCubit(DBClient dbClient)
    : _dataSourceLocal = InputResultDataSourceLocal(dbClient),
  ...
  void nextPage() async {
    if (_checkResult()) {
      ...
      bool isSaves = await _dataSourceLocal.addInputResult(inputData);
      if (isSaves) {
       InputDataDesign result = await _dataSourceLocal.getInputResult(_dataSourceLocal.key);
       print("Result: $result");
